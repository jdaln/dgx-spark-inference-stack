...
DOLPHIN_MISTRAL_24B_JSON="$(resolve_service_json "$ACTIVE_JSON" "dolphin-mistral-24b-venice-fp8")"
...
assert_flag_value "$JACKRONG_QWEN35_35B_A3B_JSON" "--max-num-batched-tokens" "16384" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled keeps the conservative batched-token budget"
assert_flag_value "$JACKRONG_QWEN35_35B_A3B_JSON" "--gpu-memory-utilization" "0.70" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled keeps the validated conservative memory envelope"
assert_flag_value "$JACKRONG_QWEN35_35B_A3B_JSON" "--tool-call-parser" "qwen3_coder" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled uses the Qwen coder tool parser"
assert_flag_value "$JACKRONG_QWEN35_35B_A3B_JSON" "--reasoning-parser-plugin" "/workspace/plugins/qwen3_opus_distilled_reasoning_parser.py" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled loads the shared Opus fallback reasoning plugin"
assert_flag_value "$JACKRONG_QWEN35_35B_A3B_JSON" "--reasoning-parser" "qwen3_opus_distilled" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled uses the shared Opus fallback reasoning parser"
...
assert_command_has_flag "$JACKRONG_QWEN35_35B_A3B_JSON" "--enforce-eager" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled disables distributed executor paths on the validated eager runtime"
...
assert_command_has_flag "$JACKRONG_QWEN35_35B_A3B_JSON" "--disable-uvicorn-access-log" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled suppresses uvicorn access logs"
assert_command_has_flag "$JACKRONG_QWEN35_35B_A3B_JSON" "--disable-log-stats" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled suppresses periodic stats logs"
...
assert_env_value "$JACKRONG_QWEN35_35B_A3B_JSON" "QWEN35_CHAT_TEMPLATE_MODEL_ID" "Qwen/Qwen3.5-0.8B" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled bootstraps the template installer from the cached generic Qwen checkpoint"
...
assert_volume_mount "$JACKRONG_QWEN35_35B_A3B_JSON" "$REPO_DIR/plugins" "/workspace/plugins" "jackrong-qwen3.5-35b-a3b-claude-4.6-opus-reasoning-distilled mounts the shared parser plugin directory read-only"

assert_equals "$(jq -r '.image // ""' <<<"$DOLPHIN_MISTRAL_24B_JSON")" "avarok/vllm-dgx-spark:v11-tf5" "dolphin-mistral-24b-venice-fp8 uses the dedicated Dolphin runtime image"
assert_json_value "$DOLPHIN_MISTRAL_24B_JSON" '.command[1] // ""' "/models/models--dphn--Dolphin-Mistral-24B-Venice-Edition-FP8/snapshots/26bbbeb19e545c8fe1516cb8d8db5c590184c82d" "dolphin-mistral-24b-venice-fp8 serves the pinned local snapshot path"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--runner" "generate" "dolphin-mistral-24b-venice-fp8 keeps the nonstandard generate runner"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--served-model-name" "dolphin-mistral-24b-venice-fp8" "dolphin-mistral-24b-venice-fp8 keeps the local served model id"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--max-model-len" "131072" "dolphin-mistral-24b-venice-fp8 keeps the validated 131k context ceiling"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--tool-call-parser" "dolphin_mistral" "dolphin-mistral-24b-venice-fp8 keeps the custom Dolphin tool parser"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--quantization" "compressed-tensors" "dolphin-mistral-24b-venice-fp8 keeps compressed-tensors quantization"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--kv-cache-dtype" "fp8" "dolphin-mistral-24b-venice-fp8 keeps fp8 KV cache"
assert_flag_value "$DOLPHIN_MISTRAL_24B_JSON" "--chat-template" "/workspace/tool_chat_template_mistral_parallel.jinja" "dolphin-mistral-24b-venice-fp8 uses the vendored Mistral parallel tool template"
assert_command_has_flag "$DOLPHIN_MISTRAL_24B_JSON" "--enable-auto-tool-choice" "dolphin-mistral-24b-venice-fp8 enables auto tool choice"
assert_command_has_flag "$DOLPHIN_MISTRAL_24B_JSON" "--disable-uvicorn-access-log" "dolphin-mistral-24b-venice-fp8 suppresses uvicorn access logs"
assert_command_has_flag "$DOLPHIN_MISTRAL_24B_JSON" "--disable-log-stats" "dolphin-mistral-24b-venice-fp8 suppresses periodic stats logs"
assert_volume_mount "$DOLPHIN_MISTRAL_24B_JSON" "$REPO_DIR/plugins/dolphin_mistral_tool_parser.py" "/workspace/plugins/dolphin_mistral_tool_parser.py" "dolphin-mistral-24b-venice-fp8 mounts the custom Dolphin tool parser read-only"
assert_volume_mount "$DOLPHIN_MISTRAL_24B_JSON" "$REPO_DIR/shared/tool_chat_template_mistral_parallel.jinja" "/workspace/tool_chat_template_mistral_parallel.jinja" "dolphin-mistral-24b-venice-fp8 mounts the vendored Mistral tool template read-only"
